...
const HISTORY_KEY = 'douyin_lab_history_v1';
const MAX_IMAGES = 9;
...
const buildDouyinPrompt = (params: {
  topic: string;
  imageCount: number;
  ratio: string;
  quality: string;
  ...
  return [
    '你是一位资深的抖音短视频内容策划专家，擅长爆款短视频选题与脚本设计。',
    '请输出一份完整的抖音短视频创意方案。',
    '',
    '输出要求：',
    ...
    '- title 与 content 必须是产品介绍与视频脚本方向（开场钩子+卖点+场景），不要输出提示词说明文字。',
    ...
    `分辨率：${quality}`,
    hasImages
      ? '已提供参考图：无论主题如何，文案必须聚焦参考图中的产品/主体，给出短视频卖点与脚本方向；shots 的 prompt 必须融合参考图产品与风格。'
      : '未提供参考图：请围绕主题生成内容。',
  ].join('\n');
};
...
const renderShotBlob = (shot: ShotItem) =>
  new Promise<Blob | null>((resolve) => {
    ...
    ctx.fillText('抖音灵感实验室', 48, size - 60);
...
export const DouyinLabModal: React.FC<DouyinLabModalProps> = ({
  isOpen,
  language,
  apiMode,
  availableModels,
  ...
  const handleGenerateOutline = async () => {
    ...
    const prompt = buildDouyinPrompt({
      topic: trimmed,
      imageCount: count,
      ratio,
      quality,
    ...
    try {
      ...
      const title =
        parsed.title || (trimmed ? `抖音灵感方案 · ${trimmed.slice(0, 18)}` : '抖音灵感方案');
    ...
    try {
      if (shot.image) {
        if (shot.image.startsWith('data:image/')) {
          ...
          link.download = `douyin_${shot.id}.png`;
          link.click();
          return;
        }
      ...
      <div className="w-[96%] max-w-6xl h-[88vh] bg-white/90 dark:bg-gray-900/90 border border-white/20 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-5 h-14 border-b border-gray-200/70 dark:border-white/10">
          ...
          <div className="flex items-center gap-2 text-gray-900 dark:text-white font-semibold">
            ...
            <span>{language === 'zh' ? '抖音灵感实验室' : 'Douyin Lab'}</span>
          </div>
          <button
            type="button"
            onClick={onClose}
