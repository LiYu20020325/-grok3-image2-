...
import { DouyinLabModal } from './components/tools/DouyinLabModal';
import { PddLabModal } from './components/tools/PddLabModal';
...
const AUTO_HISTORY_NAME_RE = /^(?:并发V|历史V)(\d+)-(\d+)$/i;
const WAITING_HISTORY_NAME_PREFIXES = ['等待并发中', '等待中'] as const;
const isWaitingHistoryName = (name: string) =>
  WAITING_HISTORY_NAME_PREFIXES.some((prefix) => (name || '').startsWith(prefix));
...
const App: React.FC = () => {
    ...
    (sessionId: string, laneId: string, updater: (prev: LaneState) => LaneState) => {
      ...
      setHistoryList((prev) => {
        ...
        const nextList = prev.map((item) => {
          ...
          let name = item.name;
          if (sequenceNumber && (isWaitingHistoryName(name || '') || AUTO_HISTORY_NAME_RE.test(name || ''))) {
            name = `历史V${sequenceNumber}-${updatedLanes.length}`;
          }
  ...
  useEffect(() => {
    ...
    migrated = list.map((item) => {
      ...
      if (typeof next.name === 'string' && next.name.startsWith('等待并发中')) {
        next = { ...next, name: next.name.replace(/^等待并发中/, '等待中') };
        changed = true;
      }
      ...
      const nextName = `历史V${sequenceNumber}-${laneCount}`;
  ...
  const resolveAutoHistoryName = (currentName: string, sequenceNumber: number, laneCount: number) => {
    if (isWaitingHistoryName(currentName || '')) {
      return `历史V${sequenceNumber}-${laneCount}`;
    }
  ...
  return (
    ...
    <XhsLabModal
      ...
      apiMode={apiMode}
      availableModels={availableModels}
      mainSelectedModelId={selectedModelId}
